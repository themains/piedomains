...
def get_urls_year(
    ...
    try:
        ...
        rows = data[1:] if data and len(data[0]) > 0 and isinstance(data[0][0], str) else data
        ...
        logger.info(f"Found {len(urls)} archived snapshots for {clean_domain} in {year}")
        return urls
    ...
    try:
        ...
        if clean_content:
            ...
            for element in soup.find_all(['div', 'table'], 
                                       class_=lambda x: x and 'wayback' in str(x).lower()):
                element.decompose()
...
def get_closest_snapshot(
    ...
    if isinstance(target_date, datetime):
        ...
        if not (len(target_date) == 8 and target_date.isdigit()):
            raise ValueError(f"Date string must be in YYYYMMDD format, got: {target_date}")
        date_str = target_date
    else:
        raise ValueError(f"target_date must be string or datetime, got {type(target_date)}")
    ...
    try:
        ...
        response = _retry_with_backoff(_query_availability, max_retries=2, initial_delay=1.0)
        ...
        if (data.get('archived_snapshots', {}).get('closest', {}).get('available')):
            ...
            logger.info(f"Found closest snapshot for {clean_domain}: {snapshot_timestamp}")
            return snapshot_url
        else:
