            ...
            try:
                ...
                for j, domain_data in enumerate(batch):
                    ...
                    if j < len(batch_results):
                        ...
                        all_results.append(
                            self._create_error_result(domain, "Missing in batch response")
                        )
    ...
    def _update_usage_stats(self, response: Any, duration: float) -> None:
        ...
        if hasattr(response, 'usage') and response.usage:
            ...
            estimated_cost = (
                ...
                cost_per_1k_tokens[model_key]
            )
            self.usage_stats['estimated_cost_usd'] = estimated_cost
        
        # Check cost limits
        if (self.config.cost_limit_usd > 0 and 
            self.usage_stats['estimated_cost_usd'] > self.config.cost_limit_usd):
            logger.warning(
