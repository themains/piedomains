...
class Piedomain(Base):
    ...
    @classmethod
    def get_driver(cls):
        """Get configured Chrome WebDriver instance for screenshots (DEPRECATED).
    ...
    def save_image(cls, url_or_domain: str, image_dir: str) -> tuple[bool, str]:
        """Save screenshot of URL or domain homepage (DEPRECATED).
    ...
    def extract_images(
        ...
        """Extract screenshots for domains (DEPRECATED).
    ...
    def extract_htmls(
        ...
        for url_or_domain in urls_or_domains:
            # Seeded before the try so the except handlers below always have a
            # key; parse_url_to_domain can raise, and `errors[domain]` would
            # otherwise blow up with NameError instead of recording the failure.
            ...
            try:
                ...
                error_msg = f"File I/O error: {e!s}"
                errors[domain] = error_msg
    ...
    @classmethod
    def extract_html_text(cls, offline: bool, input: list[str], html_path: str):
        ...
        for file in os.listdir(html_path):
            ...
            if (domain_name in input) or offline:
                ...
                if file.endswith(".html"):
                    ...
                    with open(f"{html_path}/{file}", encoding="utf-8") as f:
