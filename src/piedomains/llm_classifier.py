...
class LLMClassifier:
    ...
    def _response_text(response: Any) -> str:
        """Extract the assistant message text from a litellm completion.
        ...
        Raises:
            ValueError: If the response carries no choices or no content.
        """
    ...
    def classify_text(self, domains: list[str], content_dict: dict[str, str]):
        ...
        Returns:
            list[dict]: One dict per domain with keys: domain, category,
    ...
    def classify_from_paths(
        ...
        Args:
            ...
            cache_dir: Directory the ``text_path``/``image_path`` entries are
                relative to
                ...
                if mode == "text":
                    if not text_path:
                        ...
                        try:
                            ...
                            llm_results = self.classify_text([domain], content_dict)
                    ...
                    if not image_path:
                        ...
                        llm_results = self.classify_multimodal(
    ...
    def classify_from_data(
        self, collection_data: dict, output_file: str | None = None, mode: str = "text"
    ) -> list[dict]:
