...
class PooledHTTPClient:
    ...
    def __init__(self):
        """Initialize the client without opening a session."""
        self._session = None
    ...
    def get(
        ...
        raise requests.exceptions.RequestException(
            f"No HTTP attempt was made for {url}: max_retries={self._config.max_retries}"
        )
    ...
    def __enter__(self):
        """Enter the context, returning this client."""
        return self

    def __exit__(self, exc_type, exc_val, exc_tb):
        """Exit the context, closing the pooled session."""
        self.close()
