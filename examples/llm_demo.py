...
def main():
    ...
    classifier.configure_llm(
        ...
        api_key=api_key,
        categories=["news", "shopping", "social", "technology", "entertainment", "education"],
    ...
    try:
        ...
        for _, row in result.iterrows():
            print(f"  {row['domain']}: {row['category']} (confidence: {row['confidence']:.2f})")
...
def demo_different_providers():
    ...
    providers = [
        ...
        {"provider": "anthropic", "model": "claude-3-5-sonnet-20241022", "env_var": "ANTHROPIC_API_KEY"},
    ...
    for config in providers:
        ...
        if api_key:
            ...
            try:
                ...
                if not result.empty:
                    row = result.iloc[0]
                    print(f"  Result: {row['category']} (confidence: {row['confidence']:.2f})")
