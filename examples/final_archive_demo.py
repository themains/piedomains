...
for i, case in enumerate(test_cases, 1):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            predicted = (
                max(scores, key=scores.get) if any(scores.values()) else "recreation"
            )
            ...
            print(
                f"  {symbol} Content: {len(content)} chars, Title: {title_text[:40]}..."
            )
            print(
                f"  {symbol} Predicted: {predicted} (expected: {expected}) conf: {confidence:.3f}"
            )
...
if successful_count > 0:
    ...
    avg_length = sum(r["content_length"] for r in success_results) / len(
        success_results
    )
...
print(
    'result = classifier.classify(["google.com", "amazon.com"], archive_date="20200101")'
)
